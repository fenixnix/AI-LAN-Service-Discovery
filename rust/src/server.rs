...
use crate::protocol::{
    build_announce, build_discover_res, parse_message,
    DISCOVER_REQ,
};
...
impl DiscoveryServer {
    ...
    async fn run_loop(socket: UdpSocket, config: ServiceConfig, mut shutdown_rx: broadcast::Receiver<()>) {
    ...
    async fn handle_message(
        ...
        if cmd == DISCOVER_REQ {
            let query_id = payload.get("query_id")
                .and_then(|v| v.as_str())
                .unwrap_or("");
